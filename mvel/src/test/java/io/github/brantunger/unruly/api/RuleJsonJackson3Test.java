...
class RuleJsonJackson3Test {
    ...
    private static List<Rule> read(ObjectMapper mapper, String json) {
    ...
    @Test
    @DisplayName("one mix-in with both annotations reads no fields, which is why the docs show two")
    void combinedMixInFails() {
        ObjectMapper combined = JsonMapper.builder().addMixIn(Rule.class, CombinedMixIn.class).build();

        // Jackson 3 ignores unknown properties by default, so the builder gets no fields, and the rule has no name.
        DatabindException ex = assertThrows(DatabindException.class, () -> read(combined, JSON));
        assertTrue(ex.getMessage().contains("ruleName must not be null"), ex.getMessage());
    }
}
