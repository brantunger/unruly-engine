 ...
 * after another, and every condition and action of that language in the run gets the same session. The engine keeps
 * idle copies for later runs, and closes a session once it no longer needs it: when it drops a copy of the rules, when
 * {@code load()} has replaced the rules and no run is still using them, or when the engine is closed.
 * </p>
 *
 * <p>
